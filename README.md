...
## P&P Link
本协议经过多次优化目前效果较好，其优势有端到端极低延迟：约 5~10ms（主循环 100Hz，串口 115200bps，单帧最小 7 字节 约0.6ms 传输）、稳定运行频率主循环 100Hz，心跳超时 3000ms，无 ACK 开销，纯单工发送无拥塞控制、协议仅用作挂载控制链路（子设备 PWM 转发 + 心跳），不承担飞控与地面站主链路通信
完整帧结构为：
